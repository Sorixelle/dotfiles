...
{
  options = let inherit (lib) types mkOption mkEnableOption;
  in {
    srxl.email = {
      enable =
        mkEnableOption "configuration for accessing and synchronizing my email";
        ...
        shortcutType = types.submodule {
          options = {
            ...
            hidden = mkOption {
              ...
              default = false;
              description =
                "Whether to hide the shortcut on the mu4e main menu.";
            };
          };
        };
      in mkOption {
        ...
        default = [ ];
        description =
          "A list of shortcuts to create within mu4e, if Emacs is setup.";
      };
    };
  };

  config = let conf = config.srxl.email;
