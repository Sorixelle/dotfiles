...
Cool. Now I can use that ~poly-elixir-mode~ to handle Elixir files. The ~require~ is there for the same reason as before.
...
#+begin_src emacs-lisp
  (sp-local-pair 'elixir-ts-mode "~H" "\"\"\"" :post-handlers '("\"\"\"\n||\n[i]"))
#+end_src
