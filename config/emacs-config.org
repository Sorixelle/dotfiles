...
Cool, now I can load the theme.

#+begin_src emacs-lisp
  (load-theme srxl/theme-name t)
#+end_src

If Catppucin is the selected theme, for some reason that one needs a reload when new Emacs client frames are created, otherwise some of the colours are all wrong. Not really sure why. It's weird.

#+begin_src emacs-lisp
  (when (eq srxl/theme-name 'catppuccin)
...
The =org-indent= face is in a separate package from the main Org one, so it gets it's own =use-package= declaration.
