...
Everyone has files on their computer. Everyone needs a file browser to go find those files. And of course, Emacs has you sorted there, with Dired. It's a directory editor, hence the name. And it's a pretty powerful one too - there's very few things you can't do with it. Bulk copy/cut/paste, set file permissions, make a hardlink - you name it, Dired can do it.

But, by default, it's not very pretty. It's basically just a glorified ~ls~ output, which like... you could do worse. You could also do better though, and [[https://github.com/alexluigit/dirvish][Dirvish]] does just that. Instead of reimplementing a whole new file browser, it just piggybacks off of Dired, giving it a much nicer coat of paint. If you've ever used [[https://github.com/ranger/ranger][Ranger]], it's very much inspired by it.
...
The stock config isn't super usable in my experience, so I'm gonna have to do a bit of configuration to get it just right. First, I'll set the right ~ls~ flags for getting a directory listing. Human-readable sizes, and sort the folders first.

#+begin_src emacs-lisp :tangle no :noweb-ref dirvish-config
  (setq dired-listing-switches "-lhG --group-directories-first")
#+end_src

Next, the attributes. Dirvish can display other useful information in the file listing, like version control status and file format icons. I'll turn those on, along with an indicator for expanded folders (another neat Dirvish feature!), the last Git commit message that touched the file (if I'm in a Git repo), and file permissions and size.

#+begin_src emacs-lisp :tangle no :noweb-ref dirvish-config
  (setq dirvish-attributes '(vc-state subtree-state nerd-icons git-msg file-modes file-size))
#+end_src

The icons require the ~nerd-icons~ package to be installed, so I'll make sure I've got that.

#+begin_src emacs-lisp
  (use-package nerd-icons)
#+end_src

I'll also want my keybindings, so I can get to Dirvish and do what I need in there.

#+begin_src emacs-lisp :tangle no :noweb-ref dirvish-config
  (srxl/leader-def "d" '("Browse files" . dirvish))
  (general-def 'normal dirvish-mode-map
    "q" '("Quit" . dirvish-quit)

    "TAB" '("Toggle expand folder" . dirvish-toggle-subtree)
    "RET" '("Open file" . dired-find-alternate-file)
    "F" '("Create file" . dired-create-empty-file)
    "D" '("Create folder" . dired-create-directory)
    "h" '("Open parent directory" . dired-up-directory)
    "l" '("Enter selected entry" . dired-find-file)
    "r" '("Rename" . dired-do-rename)
    "x" '("Delete" . dired-do-delete)

    ";" '("Switch layout" . dirvish-layout-switch)
    ":" '("Fullscreen" . dirvish-layout-toggle))
#+end_src

I'd like to be able to toggle hidden dotfiles, so I can jump into them when I need them, and keep the clutter out when I don't. Doing that is just a matter of adding ~-A~ to the ~ls~ flags, and ~dired-sort-other~ is just the trick for updating those flags. I'll write a function that toggles it's addition on and off, and bind a key to call that function.

#+begin_src emacs-lisp :tangle no :noweb-ref dirvish-config
  (defun srxl/dired-toggle-hidden-files ()
    "Toggles the visibility of hidden files in the current Dired buffer."
    (interactive)
    (dired-sort-other (if (string-match-p "-A" (or dired-actual-switches ""))
                          dired-listing-switches
                        (concat dired-listing-switches " -A"))))
  (general-def 'normal dirvish-mode-map
    "." '("Toggle hidden files" . srxl/dired-toggle-hidden-files))
#+end_src

Dirvish also comes with a ~dirvish-side~ view, which is great for working in my code projects - think like your usual IDE file listing sidebar. By default, it's supposed to open at the root of the current project if there is one (I'll get to [[*Project management][projects]] in a moment), but it doesn't seem to work if the file isn't tracked by version control, making it fall back to the current buffer's directory instead. To work around that, I'll write a simple wrapper that calls ~dirvish-side~ with the project's root path, or just leave it to default if I'm not in a project. That'll get bound into the leader keymap. I'll also remove most of the noisy attributes in there so it doesn't get inevitably cut off.

#+begin_src emacs-lisp :tangle no :noweb-ref dirvish-config
  (defun srxl/dirvish-side ()
    "Open Dirvish's side panel in the current project's root, or buffer directory if there is no current project."
    (interactive)
    (dirvish-side (if-let* ((project (project-current)))
                      (project-root project)
                    nil)))

  (srxl/leader-def "." '("Show files sidebar" . srxl/dirvish-side))
  (setq dirvish-side-attributes '(vc-state subtree-state nerd-icons))
#+end_src

Earlier, I rebound =RET= to ~dired-find-alternate-file~, so it closes the side panel when I open a file. That prints a warning because closing the current buffer is apparently confusing behaviour or something? I'll shut that up.
...
[[https://docs.emmet.io/][Emmet]] defines some handy shortcuts for writing HTML and CSS. [[https://github.com/smihica/emmet-mode][emmet-mode]] implements them in Emacs.
...
I gotta tell ~emmet-mode~ that these treesit-based modes should be subject to JSX changes, like using ~className~ instead of ~class~. The treesit modes aren't in there for some reason.

#+begin_src emacs-lisp :tangle no :noweb-ref emmet-mode-config
...
*** ESLint
[[https://eslint.org/][ESLint]] is a pretty popular linter for web projects - helps catch common errors and keeps your code nice and clean. [[https://github.com/orzechowskid/flymake-eslint][flymake-eslint]] brings ESLint's diagnostics into Emacs. I'll configure it for all of my web development modes (ie. anything that ESLint can lint), and configure a few bits to make sure it works.

#+begin_src emacs-lisp
  (use-package flymake-eslint
    :config
    <<flymake-eslint-config>>)
#+end_src

[[https://github.com/mantoni/eslint_d.js][eslint_d]] is a wrapper around ESLint to run it regularly in the background, giving it a massive speed boost. I usually prefer to use that in my projects, so I'll use it here too.

#+begin_src emacs-lisp :tangle no :noweb-ref flymake-eslint-config
  (setopt flymake-eslint-executable-name "eslint_d")
#+end_src

The binary might not get added to ~exec-path~ until direnv has done it's work, so I'll defer the binary check to make sure it doesn't happen until then.

#+begin_src emacs-lisp :tangle no :noweb-ref flymake-eslint-config
  (setopt flymake-eslint-defer-binary-check t)
#+end_src

Use the JSON output mode to get more accurate error reporting.

#+begin_src emacs-lisp :tangle no :noweb-ref flymake-eslint-config
  (setopt flymake-eslint-prefer-json-diagnostics t)
#+end_src

Eglot unfortunately likes to blow out all of the existing Flymake backends when it rocks up, so I need to make sure the ESLint backend gets enabled after Eglot is setup. ~eglot-managed-mode-hook~ is the point to do that, so in there, I'll add a function that enables ESLint checks if it's one of the modes it makes sense for.

#+begin_src emacs-lisp
  (add-hook
   'eglot-managed-mode-hook
   (lambda () (when (memq major-mode '( js-ts-mode
                                        typescript-ts-mode
                                        tsx-ts-mode
                                        svelte-mode
                                        astro-ts-mode))
...
*** Elixir
[[https://elixir-lang.org/][Elixir]] is a pretty niche thingy, but oooooh boy I like it. It's a Erlang/BEAM based language, which is really funky - don't see a lot of those very often. The BEAM model is absolutely fascinating to me, and I think it lends itself incredibly well to building reliable, self-healing apps that fail often, fail fast, and recover quickly. Definitely a lot of fun for backend apps. There's a built-in Tree-sitter mode for it, so I'll use that - at least partially.

I'd use ~elixir-ts-mode~, but it's support for HEEx template highlighting, as well as ~heex-ts-mode~'s, are both not very good. It's a shame, because sometimes I'm writing [[https://www.phoenixframework.org/][Phoenix]] apps, which make heavy use of them for generating HTML. ~web-mode~ does a much better job of highlighting HEEx, and since I pulled it in earlier for [[*Svelte][Svelte]], it'll be no issue using it here as well.

The ~require~ might look a bit strange here, but it's needed because ~elixir-ts-mode~ requires ~heex-ts-mode~, which adds an entry to ~auto-mode-alist~ for ~.heex~ files for it's own mode, overriding the assignment here. Annoying, but it's needed to make sure HEEx files use my ~heex-mode~.
...
Now, that solves the problem for HEEx templates in their own file, but I'm still stuck with a pretty shitty situation for inline templates in Elixir files. Not to worry though - using [[https://polymode.github.io/][Polymode]], I can define a variant of ~elixir-ts-mode~ that uses the ~heex-mode~ I just defined earlier to manage those inline sections. Polymode's pretty handy when you're dealing with languages embedded in other ones, and need to have different major modes handling different parts of a buffer.
...
#+begin_src emacs-lisp
  ...
  (define-innermode poly-elixir-heex-innermode
    :mode 'heex-mode
    :head-matcher "~H\"\"\""
    :tail-matcher "\"\"\""
    :head-mode 'host
  ...
  (define-polymode poly-elixir-mode
    ...
    :innermodes '(poly-elixir-heex-innermode))
#+end_src

Cool. Now I can use that ~poly-elixir-mode~ to handle Elixir files.

#+begin_src emacs-lisp
  (add-to-list 'auto-mode-alist '("\\.exs?\\'" . poly-elixir-mode))
  (add-hook 'poly-elixir-mode-hook 'eglot-ensure)
  (srxl/set-default-docs 'poly-elixir-mode-hook '("elixir~1.18"))
#+end_src
...
#+begin_src emacs-lisp
  (add-to-list 'auto-mode-alist '("mix\\.lock\\'" . poly-elixir-mode))
#+end_src

I'll make sure Apheleia uses ~mix format~ to format HEEx files too - otherwise, it'll try to use the default for ~web-mode~ (ie. Prettier) and that won't end well.

#+begin_src emacs-lisp :tangle no :noweb-ref apheleia-config
  (add-to-list 'apheleia-mode-alist '(heex-mode . mix-format))
#+end_src
...
#+begin_src emacs-lisp :tangle no :noweb-ref eglot-config
  (add-to-list 'eglot-server-programs '(poly-elixir-mode "elixir-ls"))
  (add-to-list 'eglot-server-programs '(heex-mode "elixir-ls"))
#+end_src
...
#+begin_src emacs-lisp
  (add-hook 'poly-elixir-mode-hook
            (lambda () (setq-local
                        eglot-ignored-server-capabilities
                        '(:documentOnTypeFormattingProvider))))
#+end_src

HEEx templates have syntax for embedding Elixir expressions between ~<% %>~ tags, along with [[https://hexdocs.pm/eex/EEx.html#module-tags][some other variants]]. I wouldn't mind being able to autocomplete them with Smartparens like most other pairs of braces, so I'll define a pair for HEEx files, with some post handlers to ensure nicely formatted whitespace when entering them (and proper completion at the end for comments).
...
The =~H= sigil in Phoenix is a common way to create a HEEx template inline in Elixir code from a multi-line string, so I'll define a pair to insert those quickly as well.

#+begin_src emacs-lisp
  (sp-local-pair 'poly-elixir-mode "~H" "\"\"\"" :post-handlers '("\"\"\"\n||\n[i]"))
#+end_src
...
*** Justfiles
Kinda like Makefiles, but [[https://just.systems/][just]] a little bit better for running simple scripts. I like to keep one around in my projects, as shortcuts for common commands.
