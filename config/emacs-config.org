...
Since I have a couple of email addresses that I send from, I want to be able to switch between them when needed. Contexts is how you do this in mu4e - create a context for each "identity", and switch between them based on where the email is located. Simple. =match-func= will do a check on the message to see which context should be used - most of the time, it'll just check what mail folder the email is in.
