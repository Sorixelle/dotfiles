...
Dirvish also comes with a ~dirvish-side~ view, which is great for working in my code projects - think like your usual IDE file listing sidebar. I'll bind that to another key, and remove most of the noisy attributes in there so it doesn't get inevitably cut off.

#+begin_src emacs-lisp :tangle no :noweb-ref dirvish-config
  (srxl/leader-def "." '("Show files sidebar" . dirvish-side))
