...
Some parts of Org documents should still be displayed with a fixed pitch face, though, like tables and code blocks. So I should make sure those elements still display correctly, no matter what.

#+begin_src emacs-lisp :tangle no :noweb-ref org-pre
  (defun srxl/setup-org-mode-faces ()
    "Correctly setup fixed-pitch fallbacks for some Org-mode faces."
    ...
    (remove-hook 'server-after-make-frame-hook #'srxl/setup-org-mode-faces))
#+end_src

This needs to be run directly after the code to [[*Colours][set the theme]], since the theme might override these faces. Adding a positive depth should ensure it runs after the hook specified there. Same strategy as before, other than that.

#+begin_src emacs-lisp :tangle no :noweb-ref org-config
  (add-hook 'server-after-make-frame-hook #'srxl/setup-org-mode-faces 20)
  (unless (daemonp)
    (srxl/setup-org-mode-faces))
#+end_src
...
Starting mu4e is as simple as calling the =mu4e= function. I'll chuck that on a leader key definition for easy access.

#+begin_src emacs-lisp :tangle no :noweb-ref mu4e-config
  (srxl/def-leader-key "e" #'mu4e "View emails")
#+end_src
...
Eglot also doesn't have associations for this one built in, so I'll define them here.

#+begin_src emacs-lisp :tangle no :noweb-ref eglot-mode
