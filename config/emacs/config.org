...
Cool. Now, in case you didn't know, I use [[https://nixos.org/][NixOS]] as my operating system, and I have a [[https://github.com/Sorixelle/dotfiles/][hella fancy setup]] for managing every single little piece of my machines. Including Emacs. I've written a [[https://github.com/Sorixelle/dotfiles/blob/main/home/modules/emacs.nix][cool little module]] to expose some parts of that configuration to Emacs in the form of an additional variables file. Let's load that in now.
#+begin_src emacs-lisp :tangle yes
...
[[https://github.com/oantolin/embark][Embark]] compliments this setup nicely by giving us a way to perform actions on a completion target, among other things. We'll bind it to =C-SPC=, and setup it's integration with Consult.
#+begin_src emacs-lisp :tangle yes
  (use-package embark
    :bind
    (("C-SPC" . embark-act)))
...
- Inbox :: If I've been rotating an idea in my mind for a bit, it goes into here. Eventually, it'll get refined into a proper note, once I figure out where it should go.
...
#+begin_src emacs-lisp :noweb-ref ORG-ROAM-CONFIG
  (setq org-roam-capture-templates '(("i" "Inbox" entry "* ${title}\n%?"
...
For my reference notes, those should have some sort of reference backing them. Obviously. [[https://github.com/emacs-citar/citar][Citar]] provides a good interface for managing citations, as well as solid integration with Org-roam, as we'll see later.
#+begin_src emacs-lisp :tangle yes
  (use-package citar
    :hook
...
I use [[https://www.zotero.org/][Zotero]] with the [[https://retorque.re/zotero-better-bibtex/][Better BibTeX]] plugin to store all my references, so we'll make sure Org-cite and Citar are both pointed at the BibTeX export that system creates.
#+begin_src emacs-lisp :noweb-ref CITAR_CONF
  (let ((bib-file (list (expand-file-name (concat srxl/roam-dir "references/citations.bib")))))
    (setq org-cite-global-bibliography bib-file
          citar-bibliography bib-file))
#+end_src

Tell Org-cite that it should be calling out to Citar to handle adding, opening, displaying citations in Org documents.
#+begin_src emacs-lisp :noweb-ref CITAR_CONF
...
And now we get to the Org-roam integration - this allows us (among a few other things) to quickly create a note from a reference in Zotero. Neat!
#+begin_src emacs-lisp :tangle yes
...
Oh, and remember Embark from earlier? There's integration with that too, allowing us to perform a bunch of actions on a citation really easily, like opening the file or URL that it points to.
#+begin_src emacs-lisp :tangle yes
...
#+begin_src emacs-lisp :noweb-ref ORG-ROAM-CONFIG
  (srxl/def-leader-key
    "n"   '(:ignore t :wk "Notes")
    "n c" '(org-cite-insert :wk "Insert citation")
    "n f" '(org-roam-node-find :wk "Find note")
    "n l" '(org-roam-node-insert :wk "Insert link to note")
    "n n" '(org-roam-capture :wk "New note")
    "n r" '(citar-open :wk "Open reference")
...
Mu4e's default "delete email" behaviour marks a message as Trashed, which mbsync interprets as "mark for expunge", and results in mail being permanently deleted, instead of moved to the trash folder. That's no good. We should fix that up so it just moves mail to the trash, without marking it as trashed.
#+begin_src emacs-lisp :noweb-ref MU4E-CONFIG
...
Treemacs /claims/ it won't use a child frame to read input if we're in Wayland, since there's issues focusing child frames there. That doesn't seem to be the case though, so we'll force it to use the minibuffer anyway
#+begin_src emacs-lisp :noweb-ref TREEMACS-CONFIG
...
Looks like there's a bug in tsx-ts-mode at the moment. Somehow, a broken treesit query managed to slip through the cracks, breaking a lot of font locking for TSX buffers, especially around JSX tags. Annoying. I'll have to redefine the function with the fixed query for now. Looks like it's [[https://git.savannah.gnu.org/cgit/emacs.git/commit/?h=emacs-29&id=ee043a2703d3175a7745c111146aa19a13f8e0ba][fixed upstream]] though, so I guess we just wait.
