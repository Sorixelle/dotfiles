...
The leader key definition goes under =exwm-mode-map=, because =exwm-input-global-keys= isn't great for tacking a keymap onto, and we probably don't need the leader key in char mode anyway.
