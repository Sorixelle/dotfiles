...
Sometimes, unicode characters just pick whatever the hell font it wants, which is annoying. We'll force it to our monospace font, so we minimize the amount of weird spacing issues.

#+begin_src emacs-lisp :tangle yes
  (set-fontset-font t 'unicode "IBM Plex Mono")
#+end_src
...
Working across multiple projects gets real clunky, real fast. Thankfully, we have [[https://github.com/nex3/perspective-el][perspective]] to help us manage the load, by constraining window layouts and buffer lists to "perspectives", and letting us switch between them. Just like workspaces.
...
First off, some keybinds.  We're mostly going to replace a lot of the default "switch buffer" type commands with ones that are perspective-aware, so we get the buffer constraining behaviour by default.

#+begin_src emacs-lisp :noweb-ref PERSP-CONFIG
  (general-def
    "C-x b"   'persp-ivy-switch-buffer
    "C-x C-b" 'persp-list-buffer)
#+end_src

And now a whole bunch more under the leader key. These probably won't see heavy use, but it's still good to have them around.

#+begin_src emacs-lisp :noweb-ref PERSP-CONFIG
  (srxl/def-leader-key
    "p" '(:ignore t :wk "Perspectives...")
    "p a" '(persp-add-buffer :wk "Add buffer to persp")
    "p k" '(persp-remove-buffer :wk "Remove buffer from persp")
    "p m" '(persp-merge :wk "Merge current persp with other")
    "p r" '(persp-rename :wk "Rename persp")
    "p s" '(persp-switch :wk "Switch persp")
    "p x" '(persp-kill :wk "Delete persp"))
#+end_src

Get that little warning about not defining =persp-mode-prefix-key= to shut up. We defined the keybindings ourself, it's fine. Shush.

#+begin_src emacs-lisp :noweb-ref PERSP-CONFIG
  (setq persp-suppress-no-prefix-key-warning t)
#+end_src

When we're dealing with code projects, you really wanna be focusing on one at a time. Perspective's workspace-like isolation is perfect for this. [[https://github.com/bbatsov/persp-projectile][persp-projectile]] gives us a nice integration that lets us switch to a project and create a perspective for it at the same time. Exactly the kind of integration we want.

#+begin_src emacs-lisp :tangle yes
  (use-package persp-projectile
    :config
    (srxl/def-leader-key
      "s" '(projectile-persp-switch-project :wk "Switch project")))
#+end_src

It looks like there's currently [[https://github.com/nex3/perspective-el/issues/195][a bit of a bug]] in this package that causes some strange behaviour when killing a frame that only has one dedicated window in it. Sounds like a weird edge case, I know, but this actually happens a lot in EXWM since things like floating dialog boxes are frames with one dedicated window in them. I /think/ we can work around this by just picking out the offending function from a hook when we're using EXWM. It's not like I'm making and closing a lot of new frames in EXWM, let alone using perspectives in them, so this should be safe, right? ...Right?

#+begin_src emacs-lisp :tangle yes
  (when srxl/use-exwm
        (setq delete-frame-functions (remove 'persp-delete-frame delete-frame-functions)))
#+end_src
...
Bind a couple keys to be used in all buffers under EXWM, X window or not:
- =s-SPC= :: Open an app launcher
- =s-RET= :: Open a terminal using vterm
- =M-SPC= :: Same leader key as =SPC= in normal Emacs buffers
- =M-<tab>= :: Switch to the next perspective
- =M-<backtab>= :: Switch to the previous perspective

#+begin_src emacs-lisp :noweb-ref EXWM-CONFIG
  (general-def '(exwm-mode-map global-map)
    "s-SPC" 'counsel-linux-app
    "s-<return>" 'vterm

    "M-SPC" srxl/leader-key-map
    "M-<tab>" 'persp-next
    "M-<backtab>" 'persp-prev)
#+end_src

And some keybinds for X window buffers, in both line and char mode:
...
EXWM provides workspaces, but they're a little clunky for multiple monitor setups, in my opinion. There's no great way to move a workspace to another monitor - you gotta manually poke and prod at =exwm-randr-workspace-monitor-plist= to do it, since that's the single source of truth for what workspace is on what monitor. I could write some functions to do that, but we have perspective setup - we can get most of the workspace functionality we want out of that anyway, AND we can name them anything we want instead of predefined mappings to numbers. Sounds optimal to me (for now at least. Maybe I'll get off my ass and actually write some functions to make workspaces more like how most tiling WMs do it).

With all that said - 2 workspaces by default. One for each monitor.

#+begin_src emacs-lisp :noweb-ref EXWM-CONFIG
  (setq exwm-workspace-number 2)
#+end_src

We'll also generate a few keybindings for =s-0= and =s-1= to switch to the primary and secondary monitor, respectively.
...
And for =s-S-<number>=, to move a buffer to that monitor.
...
=exwm-randr= has all the magic to make multi-monitor work, so we'll configure that as well. =exwm-randr-workspace-monitor-plist= defines a mapping of workspaces to monitor outputs, and the hook will call =xrandr= to position my monitors correctly, as well as set correct refresh rates.

#+begin_src emacs-lisp :noweb-ref EXWM-CONFIG
  (require 'exwm-randr)
  (setq exwm-randr-workspace-monitor-plist '(0 "DisplayPort-0" 1 "HDMI-A-0"))
  (exwm-randr-enable)
#+end_src
...
Throw a tinge of transparency on the frames, so we can see our pretty wallpapers a bit.
